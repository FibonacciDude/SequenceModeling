...
tasks = ["_Balura_Game", "_Reading",  "_Video_1",  "_Video_2"]#,"_Fixations"]
tasks_code = ["BLG", "TEX", "VD1", "VD2"]#, "FXS"]
    ...
    with open(dir) as csvfile:
        ...
        for i, row in enumerate(spamreader):
            ...
            if (i-1) % (config['Hz'] // config['second_split']) == 0:
                ...
                # flag = 1 when not nan else 0 (inverse flag)
                # using flag feature
                #arr = np.array([1, 1, not flag]) if flag else np.array([float(row[1]), float(row[2]), not flag])
                #arr2 = np.array([0]*(info.feature_size-1)+[info.feature_size]) if flag else np.ones(info.feature_size)
                #  not using flag feature
                arr = np.array([0, 0]) if flag else np.array([float(row[1]), float(row[2])])
                arr2 = np.array([0, 0]) if flag else np.ones(info.feature_size-1)
                vecs.append(arr)
                pads.append(arr2)
...
def get_batch(i, dataset, mask, lens, labels, type = 0, device="cuda"):
    ...
    # get batch, NOT as a packed padded sequence
    # this works for any dataset with labels of shape (batch, seq, dim) of same dimension - like zs
